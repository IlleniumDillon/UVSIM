...
class gridPathFinder
{
	private:
		...
		inline bool hasForced(const Vector3i & idx, const Vector3i & dir);

		inline bool isOccupied(const int & idx_x, const int & idx_y, const int & idx_z) const;
		inline bool isOccupied(const Vector3i & index) const;
		inline bool isFree(const int & idx_x, const int & idx_y, const int & idx_z) const;
		inline bool isFree(const Vector3i & index) const;

		inline Vector3d gridIndex2coord(const Vector3i & index) const;
		inline Vector3i coord2gridIndex(const Vector3d & pt) const;
